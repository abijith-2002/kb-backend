...
Why you might see “new row violates row-level security policy” on POST /files:
...
- The API now constructs a Supabase client that carries the Authorization bearer token for each request. This ensures DB calls run under the user’s identity and RLS `auth.uid()` is populated.
